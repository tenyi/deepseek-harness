<!-- 英文源文件由 scripts/gen-persistence-catalog.ts 生成；本中文文件是通過雙語配對維護的經評審對側。
     更新時先運行 `pnpm run gen-persistence-catalog` 更新英文，再更新本文件并運行 `pnpm run verify-translation-pairing --write docs/persistence-catalog.md` 重新記錄配對。 -->

# 會話持久化事件目錄

[English](persistence-catalog.md) | 中文

會話持久事件日志中可能出現的所有事件類型：完整持久化的 `SessionEvent` 信封，以及可通過合并擴展的 `SessionEventMap` 中的每個成員，包括 `@deepseek-ai/dsh-session` 所屬的詞匯和本倉庫中每個插件對 `@deepseek-ai/dsh-session/types` 的聲明合并，并附有源 JSDoc、完整 payload 聲明、surface 標記和聲明位置。本文檔是 [session.md](subsystems/session.zh.md)（surface 排序與 `deriveMessages()` 投影）、[persistence.md](subsystems/persistence.zh.md)（如何讓日志持久化）和 [session.md](subsystems/session.zh.md#cordis-surface) 中生成區域（實時總線接線；日志事件**不是** cordis 事件，它通過唯一的 `session/event` emit 到達監聽器）的補充。

英文源文件根據源碼生成（`scripts/gen-persistence-catalog.ts`），并由 `pnpm run verify-persistence-catalog`（`doc-sync`（文檔同步門禁）的一部分）驗證新鮮度；本中文文件作為經評審對側通過雙語配對維護。聲明塊保留源碼聲明和嵌套屬性的 JSDoc，只移除其所在接口／模塊帶來的縮進，并使用 `ts persistence-catalog` 圍欄（doc-typecheck 會跳過這些圍欄，因為聲明引用了其所屬模塊中的類型）。payload 中的類型名稱會鏈接到記錄該類型的頁面。已歸檔的 [persistence-log-catalog 記錄](../.agents/notes/archived/process/2026-07-04-persistence-log-catalog.md)記載了最初的目錄決策。

以下信封聲明組合了每個事件的 `type`、單調遞增的 `seq`、以 epoch 毫秒表示的 `time`、`data`、可選的未知類型跳過標記 `ignorable`，以及條件字段 `surfaceOp`／`sourceEventSeqs`。**surface** 表示 `SurfaceEventType` 成員：它會生成一條 LLM（大語言模型）消息，并聲明該事件如何加入 surface 列表。**log-only** 表示其他所有事件：這類記錄可持久化、可回放，但不參與派生歷史。每個 payload 均可進行 JSON 序列化（在 `Session.append` 處強制執行）。當前 writer 會寫入 `SESSION_FORMAT_VERSION`；受支持的歷史產物通過構建期靜態相鄰遷移目錄進入這套當前詞匯（參見[版本生命周期](subsystems/persistence.zh.md)）。范圍僅限本倉庫中的包；下游插件可以繼續合并其他當前版本事件類型，這些類型按設計不屬于本目錄，并且在后續格式遷移邊中需要顯式 disposition。
...
來源：[`packages/core/session/src/types.ts:404`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:412`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:434`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:465`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/agent/src/types.ts:87`](../packages/core/agent/src/types.ts)
...
來源：[`packages/preset/agent-presets/src/session.ts:28`](../packages/preset/agent-presets/src/session.ts)
...
類型：[ToolCallId](subsystems/core.zh.md)

來源：[`packages/interaction/user-approval/src/types.ts:44`](../packages/interaction/user-approval/src/types.ts)
...
來源：[`packages/interaction/user-approval/src/types.ts:55`](../packages/interaction/user-approval/src/types.ts)
...
來源：[`packages/interaction/user-approval/src/index.ts:33`](../packages/interaction/user-approval/src/index.ts)
...
來源：[`packages/core/session/src/types.ts:335`](../packages/core/session/src/types.ts)
...
類型：[TokenUsage](subsystems/llm-streaming.zh.md)

來源：[`packages/core/session/src/types.ts:321`](../packages/core/session/src/types.ts)
...
來源：[`packages/interaction/commands/src/types.ts:112`](../packages/interaction/commands/src/types.ts)
...
來源：[`packages/interaction/commands/src/types.ts:105`](../packages/interaction/commands/src/types.ts)
...
來源：[`packages/compaction/compaction/src/types.ts:72`](../packages/compaction/compaction/src/types.ts)
...
來源：[`packages/compaction/compaction/src/types.ts:82`](../packages/compaction/compaction/src/types.ts)
...
來源：[`packages/compaction/compaction/src/types.ts:24`](../packages/compaction/compaction/src/types.ts)
...
類型：[ContentBlock](subsystems/core.zh.md) · [TokenUsage](subsystems/llm-streaming.zh.md)

來源：[`packages/compaction/compaction/src/types.ts:34`](../packages/compaction/compaction/src/types.ts)

### `deliverables/*`

<a id="deliverablespresented--log-only"></a>

#### `deliverables/presented` — 僅日志
...
來源： [`packages/fs/tool-present/src/types.ts:15`](../packages/fs/tool-present/src/types.ts)
...
來源：[`packages/feedback/message-feedback/src/types.ts:55`](../packages/feedback/message-feedback/src/types.ts)
...
來源：[`packages/feedback/message-feedback/src/types.ts:53`](../packages/feedback/message-feedback/src/types.ts)
...
來源：[`packages/feedback/command-feedback/src/types.ts:40`](../packages/feedback/command-feedback/src/types.ts)
...
來源：[`packages/api/session-controller/src/types.ts:40`](../packages/api/session-controller/src/types.ts)
...
來源：[`packages/interaction/permission-presets/src/index.ts:54`](../packages/interaction/permission-presets/src/index.ts)
...
來源：[`packages/plan/plan-mode/src/index.ts:47`](../packages/plan/plan-mode/src/index.ts)
...
來源：[`packages/core/session/src/types.ts:377`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:365`](../packages/core/session/src/types.ts)
...
類型：[ScheduleChange](subsystems/schedule.zh.md)
...
來源：[`packages/core/session/src/types.ts:400`](../packages/core/session/src/types.ts)
...
類型：[SessionTitleEventData](subsystems/session-title.zh.md)

來源：[`packages/session/session-title/src/index.ts:77`](../packages/session/session-title/src/index.ts)
...
類型：[SessionTitleLlmRequestEventData](subsystems/session-title.zh.md)

來源：[`packages/session/session-title-llm/src/index.ts:45`](../packages/session/session-title-llm/src/index.ts)
...
來源：[`packages/session/session-log-deepseek/src/types.ts:81`](../packages/session/session-log-deepseek/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:289`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:287`](../packages/core/session/src/types.ts)
...
來源：[`packages/subagent/subagent/src/catalog.ts:40`](../packages/subagent/subagent/src/catalog.ts)
...
來源：[`packages/subagent/subagent/src/descriptor.ts:38`](../packages/subagent/subagent/src/descriptor.ts)

<a id="subagentmodel-selection-policy--log-only"></a>

#### `subagent/model-selection-policy` — 僅日志
...
來源：[`packages/subagent/tool-subagent/src/model-selection-state.ts:17`](../packages/subagent/tool-subagent/src/model-selection-state.ts)
...
來源：[`packages/core/session/src/types.ts:310`](../packages/core/session/src/types.ts)
...
類型：[TeamId](subsystems/agent-team.zh.md) · [TeamMemberSnapshot](subsystems/agent-team.zh.md)

來源：[`packages/experimental/agent-team/src/types.ts:221`](../packages/experimental/agent-team/src/types.ts)
...
類型：[TeamId](subsystems/agent-team.zh.md) · [TeamMessageId](subsystems/agent-team.zh.md)

來源：[`packages/experimental/agent-team/src/types.ts:227`](../packages/experimental/agent-team/src/types.ts)
...
類型：[TeamId](subsystems/agent-team.zh.md) · [TeamMessageSnapshot](subsystems/agent-team.zh.md)

來源：[`packages/experimental/agent-team/src/types.ts:225`](../packages/experimental/agent-team/src/types.ts)
...
類型：[TeamId](subsystems/agent-team.zh.md) · [TeamTaskSnapshot](subsystems/agent-team.zh.md)

來源：[`packages/experimental/agent-team/src/types.ts:223`](../packages/experimental/agent-team/src/types.ts)
...
類型：[TodoItem](subsystems/todo.zh.md)

來源：[`packages/todo/tool-todo/src/types.ts:31`](../packages/todo/tool-todo/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:341`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:353`](../packages/core/session/src/types.ts)
...
類型：[TurnEndReason](subsystems/session.zh.md)

來源：[`packages/core/session/src/types.ts:285`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:276`](../packages/core/session/src/types.ts)
...
來源：[`packages/core/session/src/types.ts:297`](../packages/core/session/src/types.ts)
