...
`@deepseek-ai/dsh-*` harness 包之間的 peer 依賴關系。peer 表示消費端需要提供共享實例，不包括普通運行時 dependency 或僅開發期關系。該圖按 `packages/<group>/<pkg>` 層級分組；邊 `a --> b` 表示包 `a` peer 依賴包 `b`。名稱中的 `@deepseek-ai/dsh-` 前綴已移除。
