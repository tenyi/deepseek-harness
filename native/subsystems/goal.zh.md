...
事件溯源目標服務及其策略消費方共享的類型。[目標領域 Agent Note](../../.agents/notes/implemented/feature/2026-07-19-persisted-same-session-goal-domain.zh.md) 負責記錄持久化與激活決策；本頁記錄 [`packages/goal/goal/src/types.ts`](../../packages/goal/goal/src/types.ts) 中的確切字段和變體。

## 標識與生命周期

`GoalId` 是[品牌化 id](core.zh.md#branded-ids)。調用方通過 `GoalRef` 修改一個確切修訂版本；每次獲準的持久變更都會遞增修訂號。
...
持久階段回答目標發生了什么。進程本地激活狀態則另行回答續跑消費方能否開始另一個 Round。
...
阻塞是唯一表示「因問題而停止」的持久狀態。由策略負責的阻塞原因會攜帶一個用于路由、穩定且采用 lower-kebab-case 的代碼，以及一段供人和模型閱讀的自由文本說明。
...
服務還會在不改變持久狀態的情況下發布進程本地 activation 邊沿；客戶端消費該事件獲得實時狀態。
...
每次變更都是持久的 `goal/change` 會話事件，其載荷要么是變更后的完整快照，要么是清除墓碑。嚴格折疊與持久投影只從這些事件派生生命周期狀態；inbox 變更不會影響 goal 狀態。
...
續跑消費方會為每個獲準的用戶消息輪次標注正數且連續的 Round 編號和當前修訂號；只有這些獲準的 `user/message` 事件會推進 `roundsStarted`。回放會拒絕非正數 Round、編號缺口、陳舊修訂號、已停止階段和超出上限。
...
創建操作會區分調用方省略字段與采用部署配置值這兩種情況，`create()` 會在內部解析后者。編輯是局部替換，其運行時校驗器要求至少提供一個字段。每條變更通知都會攜帶獲準的操作和確切修訂號；清除操作不帶 `goal`。
...
[`GoalService`](../../packages/goal/goal/src/index.ts) 解析創建默認值、從可選注冊的 `goal` 投影讀取嚴格回放結果、校驗傳入的 agent（智能體）是注冊表中的確切活躍實例、以比較并設置方式執行變更，并發出 `goal/changed` 通知；監聽器故障會被隔離。注冊表或 key 缺失時，第一次依賴它們的訪問會失敗。包 [README](../../packages/goal/goal/README.zh.md) 定義可調用 API 和面向模型的約定。
