...
[dsh-credentials](../../packages/credentials/credentials) 的憑據 seam 把機密擋在配置之外：settings 分節與 `cordis.yml` 條目攜帶的是*引用*（環境變量名），值歸 [dsh-credentials-local](../../packages/credentials/credentials-local) 這類提供方所有，消費方每個操作解析一次引用——LLM（大語言模型）適配器每次模型請求解析一次，因此輪換后的憑據無需任何重啟即可作用于緊隨其后的下一次請求。一條 seam 級規則約束每個提供方：空的存儲值在任何地方都視為不存在。

來源：[`packages/credentials/credentials/src/index.ts`](../../packages/credentials/credentials/src/index.ts)

## 標識

引用以 POSIX 風格環境變量名命名一條憑據。brand 防止調用方將憑據引用與在包或進程之間傳遞的其他字符串混用；構造時校驗 shell 標識符語法。
...
`resolve(ref)` 返回值及提供該值的來源層（由提供方定義）；未配置期間返回 `undefined`。消費方在每個操作中重新解析，絕不跨操作緩存——這種按操作進行的讀取正是熱更新機制。
...
`describe(ref)` 在絕不暴露值的前提下回應配置界面：引用當前是否可解析、來自哪一層、`set` 當前能否成功。本地提供方把由當前進程環境供值的引用報告為 `writable: false`——那樣的寫入會表面成功而解析持續返回遮蔽值，因此 seam 直接拒絕，界面也得以提前把該引用渲染為只讀。
...
`credentials/reference-updated (ref)` 在提供方管理的來源發生已提交變更后發出——`set`、`unset` 或在存儲中觀察到的外部編輯。進程環境自身的變化不可觀測，永不發出事件。消費方不需要該事件（它們按操作重新解析）；它服務于配置界面刷新「已配置」徽標。
