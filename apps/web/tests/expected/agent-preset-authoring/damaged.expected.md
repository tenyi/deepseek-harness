- dialog "設置":
  - navigation:
    - text: 設置
    - button "通用設置":
      - img
      - text: 通用設置
    - button "模型":
      - img
      - text: 模型
    - button "插件":
      - img
      - text: 插件
    - button "Agent 預設":
      - img
      - text: Agent 預設
  - button "打開配置文件"
  - button "關閉":
    - img
    - text: 關閉
  - heading "Agent 預設" [level=2]
  - paragraph: 預設即一個會話的 Agent 所運行的插件組裝 —— 它的工具、提示詞與能力。復制一份既有預設改成自己的，或用「創造模式」讓 Agent 幫你創建。
  - text: 允許切換agent模式 beta
  - paragraph: 開啟后，新任務可選擇標準、PTC、創造、極簡及自定義模式；關閉后統一使用默認模式（默認為標準模式，可自定義）。僅影響新任務。
  - switch "允許切換agent模式" [checked]
  - heading "內置" [level=3]
  - list:
    - listitem:
      - 'button "新任務默認: 標準模式" [disabled] [pressed]':
        - text: 標準模式 內置 新任務默認 功能完整的編碼 Agent，支持文件編輯、Shell、文件與網頁檢索、Skills、計劃、目標、子代理和工作流。
        - code: standard
      - 'button "查看: 標準模式"':
        - img
        - text: 查看
      - 'button "復制: 標準模式"':
        - img
        - text: 復制
    - listitem:
      - 'button "設為默認: PTC 模式"':
        - text: PTC 模式 內置 功能完整的編碼 Agent，但默認不提供 workflow 工具；其他工具通過 PTC 模式 SDK 呈現，讓模型用一個 TypeScript 程序組合多步操作。
        - code: ptc
      - 'button "查看: PTC 模式"':
        - img
        - text: 查看
      - 'button "復制: PTC 模式"':
        - img
        - text: 復制
    - listitem:
      - 'button "設為默認: 極簡模式"':
        - text: 極簡模式 內置 僅提供持久 shell 的單工具編碼 Agent。
        - code: minimal
      - 'button "查看: 極簡模式"':
        - img
        - text: 查看
      - 'button "復制: 極簡模式"':
        - img
        - text: 復制
    - listitem:
      - 'button "設為默認: 創造模式"':
        - text: 創造模式 內置 用于創建自定義 Agent preset：具備標準模式的全部能力，并提供運行時檢查、插件實驗和 preset 創作指導。
        - code: cordis
      - 'button "查看: 創造模式"':
        - img
        - text: 查看
      - 'button "復制: 創造模式"':
        - img
        - text: 復制
  - heading "自定義" [level=3]
  - list:
    - listitem:
      - 'button "加載失敗: broken-yaml" [disabled]':
        - text: broken-yaml 加載失敗 自定義 暫無描述。
        - alert: "the composition is not valid YAML: unexpected end of the stream within a flow collection (3:1)"
        - code: broken-yaml
      - 'button "查看路徑: broken-yaml"':
        - img
        - text: 查看路徑
      - 'button "復制: broken-yaml" [disabled]':
        - img
        - text: 預設加載失敗，不能復制
      - 'button "刪除: broken-yaml"':
        - img
        - text: 刪除
    - listitem:
      - 'button "加載失敗: 幽靈預設" [disabled]':
        - text: 幽靈預設 加載失敗 自定義 composition 已被手動刪除。
        - alert: the composition file agent.cordis.yml is missing — the directory still occupies the id; delete it or restore the file
        - code: ghost
      - 'button "查看路徑: 幽靈預設"':
        - img
        - text: 查看路徑
      - 'button "復制: 幽靈預設" [disabled]':
        - img
        - text: 預設加載失敗，不能復制
      - 'button "刪除: 幽靈預設"':
        - img
        - text: 刪除
  - button "用「創造模式」創作自定義預設":
    - img
    - text: 用「創造模式」創作自定義預設
