...
describe('web e2e: plugin configuration section', () => {
  ...
  async function openPlugins() {
    if (await page.getByRole('dialog', { name: '設置' }).count() > 0) {
  ...
  it('shows one card per exposed host-plane namespace', async () => {
    ...
    expect(await dialog.getByRole('button', { name: '展開設置: Subagent' }).count()).toBe(1)
    await dialog.getByText('終端', { exact: true }).waitFor({ timeout: 10_000 })
    expect(await dialog.getByText('Agent 循環', { exact: true }).count()).toBe(1)
    expect(await dialog.getByText('網頁搜索', { exact: true }).count()).toBe(1)
    // Collapsed: a card's fields appear only once it is expanded.
    expect(await dialog.getByLabel('命令超時（毫秒）').count()).toBe(0)
  ...
  it('persists selected adapter routes as the subagent model allowlist', async () => {
    ...
    const toggle = dialog.getByRole('switch', { name: '允許 Agent 為 Subagent 選擇模型' })

    await toggle.click()
    const models = dialog.getByRole('group', { name: 'Agent 可選擇的模型' })
    await models.waitFor({ timeout: 10_000 })
    const firstModel = models.getByRole('checkbox').first()
    await firstModel.check()
    await dialog.getByRole('button', { name: '保存', exact: true }).click()

    const expandSubagent = dialog.getByRole('button', { name: '展開設置: Subagent' })
  ...
  it('stages an edit and writes it only when saved', async () => {
    ...
    const dialog = await openPlugins()
    await dialog.getByText('終端', { exact: true }).click()

    const timeout = dialog.getByLabel('命令超時（毫秒）')
    ...
    await expect.poll(async () => (await settingsDocument()).includes('timeoutMs: 12000'), { timeout: 10_000 })
      .toBe(true)
    const expandTerminal = dialog.getByRole('button', { name: '展開設置: 終端' })
    ...
    await expect.poll(() => dialog.getByText('已覆蓋').count(), { timeout: 5_000 }).toBe(1)
    expect(await dialog.getByRole('button', { name: '恢復默認' }).count()).toBe(1)
  ...
  it('drops a staged edit on discard without touching the document', async () => {
    ...
    await dialog.getByRole('button', { name: '放棄修改' }).click()
  ...
  it('refuses to save a draft that is not a number', async () => {
    ...
    const save = dialog.getByRole('button', { name: '保存', exact: true })
    await expect.poll(() => save.isDisabled(), { timeout: 5_000 }).toBe(true)
    expect(await dialog.getByText('請填數字；留空表示使用默認值。').count()).toBe(1)
    await dialog.getByRole('button', { name: '放棄修改' }).click()
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
  ...
  it('clears the field back to the composed default on reset', async () => {
    ...
    await dialog.getByRole('button', { name: '恢復默認' }).click()
    ...
    await expandTerminal.click()
    expect(await timeout.inputValue()).toBe('60000')
    expect(await dialog.getByText('已覆蓋').count()).toBe(0)
    expect(tripwire.pageErrors).toEqual([])
  }, 60_000)
