...
describe('TeamAction', () => {
  ...
  it('keeps a successful task mutation newer than an in-flight refresh', async () => {
    ...
    expect(await screen.findByRole('button', { name: /重開/u })).toBeTruthy()

    stale.resolve({ ok: true, value: view })
    await Promise.resolve()
    expect(screen.getByRole('button', { name: /重開/u })).toBeTruthy()
  ...
  it('keeps a created task newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: zh.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /新建任務/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'New task' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Details' } })
  ...
  it('keeps task and create failures newer than an in-flight refresh', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: zh.refresh }))
    fireEvent.click(screen.getByRole('button', { name: /新建任務/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Rejected task' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Rejected details' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
  ...
  it('tracks simultaneous create and task mutations independently', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /新建任務/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Concurrent task' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Concurrent details' } })
  ...
  it('creates a task from normalized blocker and write-scope lists', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: ' New task ' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: ' Details ' } })
    fireEvent.change(screen.getByPlaceholderText(/依賴任務/u), { target: { value: 'task-1, task-1' } })
    fireEvent.change(screen.getByPlaceholderText(/寫入范圍/u), { target: { value: 'src/a, src/b' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    await waitFor(() => {
  ...
  it('assigns, edits, completes, reopens, and deletes with contiguous CAS revisions', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /編輯/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Updated runtime' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Updated details' } })
    fireEvent.change(screen.getByPlaceholderText(/依賴任務/u), { target: { value: 'task-0' } })
    fireEvent.change(screen.getByPlaceholderText(/寫入范圍/u), { target: { value: 'src/runtime' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    ...
    fireEvent.click(screen.getByRole('button', { name: /完成/u }))
    fireEvent.click(await screen.findByRole('button', { name: /重開/u }))
    await waitFor(() => {
      expect(screen.queryByRole('button', { name: /重開/u })).toBeNull()
      expect(current).toMatchObject({ revision: 6, status: 'pending' })
    })
    fireEvent.click(screen.getByRole('button', { name: /刪除/u }))
  ...
  it('keeps reload failures visible after task and dependency conflicts', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /編輯/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Edited' } })
  ...
  it('renders roster/task state variants and contains navigation, refresh, and close actions', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /^worker運行中/u }))
  ...
  it('shows load and create failures and ignores a create result after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /新建任務/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Task' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Description' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /新建任務/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Late task' } })
    fireEvent.change(screen.getByPlaceholderText('任務描述'), { target: { value: 'Late description' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
  ...
  it('contains edit and dependency failures and supports form cancellation and unassignment', async () => {
    ...
    expect(screen.queryByPlaceholderText('任務標題')).toBeNull()
    ...
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Saved edit' } })
    fireEvent.change(screen.getByPlaceholderText(zh.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
  ...
  it('skips the dependency mutation when an edit keeps the same blockers', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /編輯/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Same dependencies' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
  ...
  it('reloads a dependency conflict and ignores dependency settlement after a session switch', async () => {
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /編輯/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Conflict edit' } })
    fireEvent.change(screen.getByPlaceholderText(zh.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    expect(await screen.findByText(zh.conflict)).toBeTruthy()
    ...
    await screen.findByText('Implement runtime')
    fireEvent.click(screen.getByRole('button', { name: /編輯/u }))
    fireEvent.change(screen.getByPlaceholderText('任務標題'), { target: { value: 'Late edit' } })
    fireEvent.change(screen.getByPlaceholderText(zh.blockers), { target: { value: 'task-2' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
