...
describe('GoalBar', () => {
  ...
  it('active goal: goal glyph, "進行中的目標", truncated objective, edit and clear actions', () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal()} {...actions} t={t} />)
    expect(screen.getByText('進行中的目標')).toBeTruthy()
  ...
  it('single-flights rapid clear clicks and hides the committed goal before its projection catches up', async () => {
    ...
    const clear = screen.getByRole<HTMLButtonElement>('button', { name: '清除目標' })
  ...
  it('edit swaps the strip for a prefilled form; Enter saves, empty stays disabled', async () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal()} {...actions} t={t} />)
    fireEvent.click(screen.getByRole('button', { name: '編輯目標' }))
    const box = screen.getByRole('textbox', { name: '目標內容' })
    expect(box).toHaveProperty('value', 'Ship the redesign')

    fireEvent.change(box, { target: { value: '   ' } })
    expect(screen.getByRole('button', { name: '保存目標' })).toHaveProperty('disabled', true)

    fireEvent.change(box, { target: { value: 'Ship v2' } })
    fireEvent.keyDown(box, { key: 'Enter' })
    expect(actions.onEdit).toHaveBeenCalledWith('Ship v2')
    await waitFor(() => { expect(screen.getByText('進行中的目標')).toBeTruthy() })
  })
  ...
  it('Esc cancels the edit without calling onEdit', () => {
    ...
    render(<GoalBar goal={makeGoal()} {...actions} t={t} />)
    fireEvent.click(screen.getByRole('button', { name: '編輯目標' }))
    fireEvent.keyDown(screen.getByRole('textbox', { name: '目標內容' }), { key: 'Escape' })
  ...
  it('the cancel button exits the form and drops the draft (re-edit starts from the objective)', () => {
    ...
    render(<GoalBar goal={makeGoal()} {...actions} t={t} />)
    fireEvent.click(screen.getByRole('button', { name: '編輯目標' }))
    fireEvent.change(screen.getByRole('textbox', { name: '目標內容' }), { target: { value: 'abandoned draft' } })
    fireEvent.click(screen.getByRole('button', { name: '取消編輯' }))
    expect(actions.onEdit).not.toHaveBeenCalled()
    expect(screen.getByText('進行中的目標')).toBeTruthy()

    fireEvent.click(screen.getByRole('button', { name: '編輯目標' }))
    expect(screen.getByRole('textbox', { name: '目標內容' })).toHaveProperty('value', 'Ship the redesign')
  })
  ...
  it('Enter with a blank draft neither saves nor closes the form', () => {
    ...
    fireEvent.keyDown(box, { key: 'Enter' })
    expect(actions.onEdit).not.toHaveBeenCalled()
    expect(screen.getByRole('textbox', { name: '目標內容' })).toBeTruthy()
  })
  ...
  it('active disarmed goal: "未運行的目標" with a resume action instead of pause', () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal()} activation="disarmed" {...actions} t={t} />)
    expect(screen.getByText('未運行的目標')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '恢復目標' }))
    expect(actions.onResume).toHaveBeenCalledTimes(1)
    expect(screen.queryByRole('button', { name: '暫停目標' })).toBeNull()
  })

  it('paused goal: "已暫停的目標" with a resume action before edit', () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal({ phase: 'paused' })} {...actions} t={t} />)
    expect(screen.getByText('已暫停的目標')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '恢復目標' }))
    expect(actions.onResume).toHaveBeenCalledTimes(1)
  })
  ...
  it('a new goal identity drops the edit form (no stale draft over the new goal)', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '編輯目標' }))
    fireEvent.change(screen.getByRole('textbox', { name: '目標內容' }), { target: { value: 'stale draft' } })
    ...
    expect(screen.queryByText('進行中的目標')).toBeNull()
  })

  it('blocked goal: "受阻的目標" with the block reason as the strip tooltip', () => {
    const actions = makeActions()
    ...
    expect(screen.getByText('受阻的目標')).toBeTruthy()
    expect(screen.getByText('受阻的目標').closest('[title]')?.getAttribute('title')).toBe('No progress in 3 rounds')
  })
  ...
  it('blocked goal without a reason carries no tooltip', () => {
    ...
    expect(screen.getByText('受阻的目標')).toBeTruthy()
    expect(screen.getByText('受阻的目標').closest('[title]')).toBeNull()
  })
  ...
  it('keeps the edit draft open and reports a failed save', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '保存目標' }))

    expect((await screen.findByRole('alert')).textContent).toBe('stale revision (session/agent-busy)')
    expect(screen.getByRole('textbox', { name: '目標內容' })).toHaveProperty('value', 'retry this draft')
  })
