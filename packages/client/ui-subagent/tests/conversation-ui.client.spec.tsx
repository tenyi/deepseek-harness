...
describe('SubagentHeaderLineage', () => {
  it('aggregates live descendant activity onto the closed trigger', () => {
    ...
    const trigger = screen.getByRole('button', { name: '1 個子代理，正在運行' })
    ...
    const inactiveTrigger = screen.getByRole('button', { name: '3 個子代理' })
  ...
  it('does not aggregate subagents reached through an ordinary fork', () => {
    ...
    const trigger = screen.getByRole('button', { name: '2 個子代理' })
  ...
  it('renders healthy counts, stable rows, diagnostics, and catalog-addressed navigation', () => {
    ...
    expect(screen.getByText('正在掃描項目文件 · 可繼續 · 正在運行')).toBeTruthy()
    expect(screen.getByText('一次性 · 當前未運行')).toBeTruthy()
    const diagnostic = screen.getByRole('treeitem', { name: /會話記錄損壞/ })
    expect(diagnostic.getAttribute('aria-disabled')).toBe('true')
    expect(screen.getByRole('button', { name: '展開 worker 的下級子代理' })).toBeTruthy()
    expect(screen.queryByRole('button', { name: '展開 reviewer 的下級子代理' })).toBeNull()
  ...
  it('removes the disclosure column from branchless catalog levels', () => {
    ...
    render(<SubagentHeaderLineage {...input} />)
    hoverCatalog(screen.getByRole('button', { name: /1 個子代理/ }))
  ...
  it('covers diagnostic variants, fallback labels, and keyboard row activation', () => {
    ...
    expect(screen.queryByRole('tree')).toBeNull()
    hoverCatalog(trigger)
    expect(screen.getByRole('treeitem', { name: /子代理記錄版本不受支持/ })).toBeTruthy()
    expect(screen.getByRole('treeitem', { name: /會話記錄暫不可用/ })).toBeTruthy()
  ...
  it('shows durable token totals, ticks active duration by seconds, and freezes inactive rows', async () => {
    ...
    render(<SubagentHeaderLineage {...input} />)
    const trigger = screen.getByRole('button', { name: '1 個子代理，正在運行' })
    expect(within(trigger).getByText('9 個子代理')).toBeTruthy()
    hoverCatalog(trigger)
    ...
    expect(screen.getByRole('treeitem', { name: /finished.*123 tok · 1小時02分03秒/ })).toBeTruthy()
    expect(screen.getByRole('treeitem', { name: /interrupted.*123M tok · 6秒/ })).toBeTruthy()
    expect(screen.getByRole('treeitem', { name: /days.*12天05小時06分07秒/ })).toBeTruthy()
    expect(screen.getByText('12天5小時').getAttribute('title'))
      .toBe('總活躍耗時：12天05小時06分07秒')
    expect(screen.getByText('1天')).toBeTruthy()
    expect(screen.getByText('約6個月12天')).toBeTruthy()
    expect(screen.getByText('約1個月')).toBeTruthy()
    expect(screen.getByText('約2年3個月')).toBeTruthy()
    expect(screen.getByText('約1年')).toBeTruthy()
  ...
  it('lazily expands and collapses descendant catalogs with direct-parent navigation', () => {
    ...
    render(<SubagentHeaderLineage {...input} />)
    hoverCatalog(screen.getByRole('button', { name: /2 個子代理/ }))

    fireEvent.click(screen.getByRole('button', { name: '展開 worker 的下級子代理' }))
  ...
  it('shows known descendant rows while their catalog loads', () => {
    ...
    const loadingRows = screen.getAllByRole('treeitem', { name: '正在加載子代理' })
    ...
    expect(screen.getAllByRole('treeitem', { name: '正在加載子代理' })).toHaveLength(2)
    ...
    expect(screen.queryByRole('treeitem', { name: '正在加載子代理' })).toBeNull()
  })
  ...
  it('closes expanded descendants even when their own catalogs have not arrived', () => {
    ...
    hoverCatalog(screen.getByRole('button', { name: /2 個子代理/ }))
    fireEvent.click(screen.getByRole('button', { name: '展開 worker 的下級子代理' }))
    fireEvent.click(screen.getByRole('button', { name: '展開 indexer 的下級子代理' }))
    fireEvent.click(screen.getByRole('button', { name: '收起 worker 的下級子代理' }))
  ...
  it('hides an arrived empty catalog and exposes retry for a failed one', () => {
    ...
    hoverCatalog(screen.getByRole('button', { name: /0 個子代理/ }))
  ...
  it('keeps known descendants reachable while their catalog is absent or stale-empty', () => {
    ...
    const trigger = screen.getByRole('button', { name: '1 個子代理，正在運行' })
    expect(within(trigger).getByText('2 個子代理')).toBeTruthy()
    hoverCatalog(trigger)
    ...
    expect(screen.getByRole('button', { name: '1 個子代理，正在運行' })).toBeTruthy()
    expect(screen.getAllByRole('treeitem', { name: '正在加載子代理' })).toHaveLength(2)
  ...
  it('hides a bare loading catalog and keeps the error fallback without focusable rows', async () => {
    ...
    render(<SubagentHeaderLineage {...failed} />)
    const trigger = screen.getByRole('button', { name: /0 個子代理/ })
    hoverCatalog(trigger)
    expect(screen.getByText('無法加載子代理')).toBeTruthy()
    fireEvent.keyDown(trigger, { key: 'ArrowDown' })
    await Promise.resolve()
    expect(screen.getByRole('tree')).toBeTruthy()
  ...
  it('places a separator before a visible ordinary-session descendant count', () => {
    ...
    expect(screen.getByRole('button', { name: /2 個子代理/ })).toBeTruthy()
  ...
  it('combines the current subagent title and chevron into the parent-catalog trigger', () => {
    ...
    const switcher = screen.getByRole('button', { name: '切換子代理：worker' })
  ...
  it('falls back to the catalog session id when the selected row has no label', () => {
    ...
    expect(screen.getByRole('button', { name: `切換子代理：${CHILD}` })).toBeTruthy()
  })
  ...
  it('keeps an ancestor switcher muted and omits its descendant count', () => {
    ...
    expect(screen.queryByRole('button', { name: /1 個子代理/ })).toBeNull()
    vi.useFakeTimers()
  ...
  it('keeps a nested title switcher scoped to its direct-parent catalog', () => {
    ...
    hoverCatalog(screen.getByRole('button', { name: '切換子代理：indexer' }))
...
describe('SubagentReadOnlyComposer', () => {
  it('explains the exact missing-parent recovery path', () => {
    ...
    expect(screen.getByRole('status').textContent).toContain('父會話當前不在線')
  })

  it('explains that one-shot histories never accept follow-ups', () => {
    render(<SubagentReadOnlyComposer matched={{ reason: 'one-shot' }} t={t} />)
    expect(screen.getByRole('status').textContent).toContain('一次性任務不支持后續消息')
  })
})
