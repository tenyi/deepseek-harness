...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    expect(screen.queryByLabelText('說明')).toBeNull()

    fireEvent.click(row)
    expect(row.getAttribute('aria-expanded')).toBe('true')
    const card = screen.getByLabelText('說明')
    expect(card.textContent).toBe('說明Follow the issue workflow.\nKeep project fields in sync.')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('正在加載 skill')
  ...
  it('uses the first failure line in the summary and exposes the full error', () => {
    ...
    const row = screen.getByRole('button', { name: 'skill 加載失敗SkillSkillError: missing resource' })
  ...
  it('renders stopped, structured, and structured-error durable outcomes', () => {
    ...
    expect(stoppedView.container.textContent).toContain('skill 加載已中止')
    ...
    const errorRow = screen.getByRole('button', { name: 'skill 加載失敗SkillSkillError: missing' })
  ...
  it('falls back to durable args or call id when the skill name is unavailable', () => {
    ...
    expect(blank.container.textContent).not.toContain('正在加載 skill')
  })
})
