...
const command = (over: Partial<CommandNode> = {}): CommandNode => ({
  ...
  name: 'plan', args: '', outcome: { kind: 'success', text: '已進入 plan mode' },
  ...over,
})
...
describe('Chat node rendering', () => {
  ...
  it('threads the injected file-mention vocabulary into the closing prose only', () => {
    ...
    const mention = view.getByRole('button', { name: '打開 site/report.html' })
  ...
  it('formatRunDuration localizes units and floors partial seconds', () => {
    ...
    expect(formatRunDuration(3_600_000, t)).toBe('1小時00分00秒')
    expect(formatRunDuration(3_903_000, t)).toBe('1小時05分03秒')
    expect(formatRunDuration(7_261_000, t)).toBe('2小時01分01秒')
  })
...
describe('ChatView', () => {
  ...
  it('projects loaded turns into prompt and response navigation previews', () => {
    ...
    const h = makeHarness({}, {}, snapshot)
    const view = render(<h.ChatView {...h.props} />)
    const navigation = view.getByRole('navigation', { name: '輪次導航' })
    expect(navigation.style.getPropertyValue('--turn-natural-height')).toBe('22px')
    const first = view.getByRole('button', { name: '跳轉到第 1 輪' })
    const second = view.getByRole('button', { name: '跳轉到第 2 輪' })
  ...
  it('jumps to a turn anchor and reflows stable marks after an older page arrives', () => {
    ...
    const movedSecond = view.getByRole('button', { name: '跳轉到第 2 輪' })
  ...
  it('extends the rail with unloaded outline turns, pages on click, and falls back when nothing lands', async () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const first = view.getByRole('button', { name: '加載并跳轉到第 1 輪' })
    view.getByRole('button', { name: '加載并跳轉到第 2 輪' })
    const third = view.getByRole('button', { name: '跳轉到第 3 輪' })
    ...
    expect(view.getByRole('button', { name: '跳轉到第 3 輪' }).getAttribute('aria-current')).toBe('true')
  })
  ...
  it('scrolls the fixed-pitch rail inside its frame with gradient fades at the scrollable ends', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const nav = view.getByRole('navigation', { name: '輪次導航' })
  ...
  it('lands a jump on its turn once the paged rows commit', async () => {
    ...
    fireEvent.click(view.getByRole('button', { name: '加載并跳轉到第 1 輪' }))
    expect(h.loadThrough).toHaveBeenCalledWith(0)
  ...
  it('prepend keeps the reader\'s latest pending-request scroll position anchored', () => {
    ...
    fireEvent.click(view.getByText('加載更早'))
  ...
  it('renders Host-pending steering at the flow tail and hands off to the durable node', () => {
    ...
    fireEvent.click(within(pendingBubble as HTMLElement).getByRole('button', { name: '復制' }))
    expect(writeText).toHaveBeenCalledWith('interrupt now')
    expect(within(pendingBubble as HTMLElement).queryByRole('button', { name: '在新對話中分支' })).toBeNull()
    ...
    expect(view.getAllByRole('button', { name: '復制' })).toHaveLength(1)
    const durableBubble = view.getByText('interrupt now').closest('[class*="userRow"]') as HTMLElement
    expect(within(durableBubble).queryByRole('button', { name: '在新對話中分支' })).toBeNull()
    ...
    const branchButtons = view.getAllByRole('button', { name: '在新對話中分支' })
        ...
        pendingSubmissions: [
          {
            ...
            time: 5_000, text: '即發即顯', attachments: [],
          },
        ],
      },
    )
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getByText('即發即顯').closest('[data-submission-echo]')).not.toBeNull()
    ...
    act(() => {
      h.setChat({
        nodes: [
          ...
          {
            kind: 'user', seq: 2, time: 2_000,
            content: [{ type: 'text', text: '即發即顯' }] as never,
            source: { kind: 'user', rpcId: 'req-1' },
          },
        ],
      })
    })
    expect(view.getAllByText('即發即顯')).toHaveLength(1)
  ...
  it('renders a local steer echo as pending steering before Host image admission completes', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const local = view.getByText('帶圖糾偏').closest('[data-submission-echo]')
    ...
    act(() => {
      h.setSession({
        queue: [{
          ...
          content: [{ type: 'text', text: '帶圖糾偏' }],
          preview: '帶圖糾偏',
          text: '帶圖糾偏',
        }],
      })
    })
    expect(view.getAllByText('帶圖糾偏')).toHaveLength(1)
    expect(view.container.querySelector('[data-submission-echo]')).toBeNull()
        ...
        pendingSubmissions: [
          {
            ...
            time: 6_000, text: '排隊中', attachments: [],
          },
        ],
      },
    )
    const view = render(<h.ChatView {...h.props} />)
    expect(view.queryByText('排隊中')).toBeNull()
    act(() => {
      h.setSession({
        queue: [{
          ...
          content: [{ type: 'text' as const, text: '排隊中' }],
          preview: '排隊中',
          text: '排隊中',
        }],
      })
  ...
  it('animates only the latest unresolved model retry', () => {
    ...
    expect(within(disclosure).getByRole('status').textContent).toBe('正在重試模型請求（1/2） · 1s')
    ...
    expect(within(disclosure).getByRole('status').textContent).toBe('正在重試模型請求（2/2） · 1s')
    ...
    expect(within(disclosure).getByRole('status').textContent).toBe('已重試模型請求（2/2） · 1s')
    ...
    expect(within(cancelledDisclosure).getByRole('status').textContent).toContain('重試已取消')
  })
  ...
  it('renders the max-tokens notice with localized guidance, distinct from turn errors', () => {
    ...
    const statuses = view.getAllByRole('status')
    expect(statuses.map(status => status.textContent)).toEqual([
      '已達到輸出 token 上限回答被截斷，已有輸出保留在對話中。發送“繼續”可讓模型接著輸出。',
    ])
    expect(view.queryByText('本輪運行失敗')).toBeNull()
  })
  ...
  it('shows assistant IconActions only on the last content message of each turn', () => {
    ...
    expect(view.getAllByRole('button', { name: '復制' })).toHaveLength(4)
    const branchButtons = view.getAllByRole('button', { name: '在新對話中分支' })
  ...
  it('folds Think and Tool rows before the final answer without unmounting them', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const toggle = view.getByRole('button', { name: '1 次工具調用 · 1 條消息 · 1 個 subagent' })
    ...
    const renewedToggle = view.getByRole('button', { name: '1 次工具調用 · 1 條消息 · 1 個 subagent' })
  ...
  it('withholds assistant IconActions while the turn is still running', () => {
    ...
    expect(view.getAllByRole('button', { name: '復制' })).toHaveLength(3)
  ...
  it('the actions-owning assistant footer shows the turn run time', () => {
    ...
    expect(view.container.querySelector('[data-turn-tail="1"]')?.textContent).toContain('用時 19秒')
  })
  ...
  it('the settled footer exposes ttft, decode throughput, and usage as the details trigger', () => {
    ...
    expect(dialog.getAttribute('aria-label')).toBe('本輪用量')
    expect(dialog.firstChild?.textContent).toBe('本輪用量10,100 tok')
    expect(dialog.textContent).toContain('緩存命中49.4%')
    expect(dialog.textContent).toContain('未緩存輸入5,060 tok')
    fireEvent.keyDown(document, { key: 'Escape' })
    // The time pill carries the run time; first-step ttft (1.2s) and 100
    // tokens over 5s of decode move into its dialog.
    const timeTrigger = view.getByRole('button', { name: /用時 19秒/ })
    expect(timeTrigger.textContent).toBe('用時 19秒')
    expect(view.queryByText(/速度 20 tok\/s|首 token/)).toBeNull()
    fireEvent.click(timeTrigger)
    const timeDialog = view.getByRole('dialog')
    expect(timeDialog.getAttribute('aria-label')).toBe('本輪用時和速度')
    expect(timeDialog.textContent).toContain('本輪總用時19秒')
    expect(timeDialog.textContent).toContain('輸出速度（TPS）20 tok/s')
    expect(timeDialog.textContent).toContain('首 token 用時（TTFT）1.2秒')
  })
  ...
  it('withholds the usage-details trigger when turn usage is outside the window', () => {
    ...
    expect(view.getByRole('button', { name: /用時/ })).toBeTruthy()
  ...
  it('the run-time label is withheld when the turn start is outside the window', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    expect(view.queryByText(/用時/)).toBeNull()
  })
  ...
  it('enables fork only on the finalized assistant at the completed transcript tail', () => {
    ...
    const buttons = view.getAllByRole('button', { name: '在新對話中分支' })
  ...
  it('disables fork when the indexed Turn has a later steering Node', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const branch = view.getByRole('button', { name: '在新對話中分支' })
  ...
  it('keeps final content actions but disables branch when Tool and interrupted Think follow it', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getAllByRole('button', { name: '復制' })).toHaveLength(2)
    const buttons = view.getAllByRole('button', { name: '在新對話中分支' })
    expect(buttons).toHaveLength(1)
  ...
  it('the running clock reads hours once the turn passes an hour', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getByRole('status').textContent).toMatch(/^深度求索中\.\.\.1小時05分0\d秒$/)
  })
  ...
  it('shows a Host open refusal with the reason and retries the same path', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('dialog', { name: '無法打開文件' })).toBeTruthy()
    })
    expect(screen.getByRole('dialog', { name: '無法打開文件' }).textContent).toContain('xdg-open is not available')
  ...
  it('keeps a non-Error Host refusal visible and dismisses it on cancel', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('dialog', { name: '無法打開文件' }).textContent).toContain('permission denied')
    })
  ...
  it('substitutes the unknown-open copy when the Host refusal has no text', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('dialog', { name: '無法打開文件' }).textContent).toContain('無法打開此文件')
    })
  })
  ...
  it('ignores a Host refusal that settles after the dialog is dismissed', async () => {
    ...
    await act(async () => { h.toolOwners[0]!.openFile('src/a.ts') })
    await waitFor(() => {
      expect(screen.getByRole('dialog', { name: '無法打開文件' }).textContent).toContain('first refusal')
    })
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '重試' })) })
    fireEvent.click(screen.getByRole('button', { name: '取消' }))
    expect(screen.queryByRole('dialog')).toBeNull()
  ...
  it('pinned dynamic-height updates select the latest Turn without reading row geometry', () => {
    ...
    expect(scroller.scrollTop).toBe(900)
    expect(view.getByRole('button', { name: '跳轉到第 2 輪' }).getAttribute('aria-current')).toBe('true')
    expect(rect).not.toHaveBeenCalled()
  })
  ...
  it('paging button loads older and shows its busy label', () => {
    ...
    expect(view.getByText('加載中…')).toBeTruthy()
  })
  ...
  it('renders command nodes as durable rows: settled text, error state, executing spinner, run-less soft-fall', () => {
    ...
    expect(view.getByText('已進入 plan mode')).toBeTruthy()
    ...
    expect(fv.getByText('指令失敗')).toBeTruthy()
    expect(fv.getByText('失敗')).toBeTruthy()
    ...
    expect(xv.getByText('執行中…')).toBeTruthy()
    expect(xv.getByText('運行中')).toBeTruthy()
  ...
  it('renders /compact as one stateful disclosure from running through completion', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getByText('正在壓縮…')).toBeTruthy()
    ...
    expect(view.queryByText('正在壓縮…')).toBeNull()
    expect(view.queryByText('上下文已壓縮')).toBeNull()
    ...
    expect(view.getByRole('heading', { name: '壓縮摘要' })).toBeTruthy()
  })
