...
describe('workspace browser rows', () => {
  ...
  it('keeps the active-Schedule marker after a search title and inside the row action', () => {
    ...
    const indicator = screen.getByRole('img', { name: '有活動定時任務' })
    expect(title.nextElementSibling).toBe(indicator)
    expect(indicator.getAttribute('title')).toBe('有活動定時任務')
  ...
  it('renders an active Workspace and keeps its create action separate from toggling', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '在“Project”中新建會話' }))
  ...
  it('keeps the active-Schedule marker between the title and time in grouped and flat rows', () => {
    ...
    const assertIndicator = (): HTMLElement => {
      ...
      const time = screen.getByText('剛剛')
      const indicator = screen.getByRole('img', { name: '有活動定時任務' })
      expect(title.nextElementSibling).toBe(indicator)
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      ...
      expect(screen.getByText('進行中')).toBeTruthy()
      expect(screen.getByText('1 個子代理運行中')).toBeTruthy()

      fireEvent.pointerEnter(row.parentElement as HTMLElement)
      act(() => { vi.advanceTimersByTime(500) })
      expect(screen.getAllByText('進行中')).toHaveLength(2)
      expect(screen.getAllByText('1 個子代理運行中')).toHaveLength(2)
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('workspace row menu opens on the ellipsis, renames, and shows the danger delete row', () => {
    ...
    />)
    fireEvent.click(screen.getByRole('button', { name: '工作區“Project”的操作' }))
    // Opening the menu neither toggles the group nor renames yet.
    expect(onToggle).not.toHaveBeenCalled()
    expect(screen.getByRole('menuitem', { name: '刪除工作區' }).className).toMatch(/danger/)
  ...
  it('workspace hover card shows its details and copies the full directory path', async () => {
    ...
    try {
      ...
      expect(screen.getByText(/^創建于 \d+年\d+月\d+日 /)).toBeTruthy()
      await act(async () => { fireEvent.click(screen.getByRole('button', { name: '復制: /projects/project' })) })
      expect(writeText).toHaveBeenCalledWith('/projects/project')
      expect(screen.getByRole('status').textContent).toBe('已復制')
    } finally {
      restoreClipboard()
      vi.useRealTimers()
    }
  ...
  it('workspace hover card shows a POSIX home descendant as ~ and still copies the full path', async () => {
    ...
    try {
      ...
      await act(async () => { fireEvent.click(screen.getByRole('button', { name: '復制: /home/u/Documents/project' })) })
  ...
  it('workspace hover card without a directory omits the path and copy action', async () => {
    ...
    try {
      ...
      expect(screen.getAllByText('Project')).toHaveLength(2)
      expect(screen.getByText(/^創建于 \d+年\d+月\d+日 /)).toBeTruthy()
      expect(screen.queryByRole('button', { name: /^復制:/ })).toBeNull()
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('blank New Session rows carry no menu, no time label, and no hover-card time', () => {
    ...
    try {
      ...
      expect(screen.queryByRole('button', { name: /會話.*的操作/ })).toBeNull()
      expect(screen.queryByText('剛剛')).toBeNull()
      // The hover card keeps title + status but drops the timestamp line.
      const wrapper = screen.getByRole('treeitem').parentElement as HTMLElement
      fireEvent.pointerEnter(wrapper)
      act(() => { vi.advanceTimersByTime(500) })
      expect(screen.getAllByText('新會話').length).toBeGreaterThanOrEqual(2)
      expect(screen.getByText('空閑')).toBeTruthy()
      expect(screen.queryByText('剛剛')).toBeNull()
      expect(screen.getByText('空閑').closest('[role="button"]')).toBeNull()
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, and archive', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '會話“One”的操作' }))
    expect(onOpen).not.toHaveBeenCalled()
    // Archive is not destructive (log and accounting slot remain): no danger styling.
    expect(screen.getByRole('menuitem', { name: '歸檔會話' }).className).not.toMatch(/danger/)
    ...
    expect(onOpen).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: '會話“One”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '分叉會話' }))
  ...
  it('shows the hover card after the dwell and suppresses it while the row menu is open', () => {
    ...
    try {
      ...
      expect(screen.getByText('1分鐘前')).toBeTruthy()
      expect(screen.getAllByText('進行中')).toHaveLength(2)
      fireEvent.pointerLeave(wrapper)
      // Menu open (disabled=true) suppresses the card for the same hover.
      fireEvent.click(screen.getByRole('button', { name: '會話“Hovered”的操作' }))
      fireEvent.pointerEnter(wrapper)
      act(() => { vi.advanceTimersByTime(1000) })
      expect(screen.queryByText('1分鐘前')).toBeNull()
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('idle hover card shows the Idle status line', () => {
    ...
    try {
      ...
      act(() => { vi.advanceTimersByTime(500) })
      expect(screen.getByText('空閑')).toBeTruthy()
      expect(screen.getAllByText('剛剛')).toHaveLength(2)
    } finally {
      vi.useRealTimers()
    }
  })
