...
function bench(over?: BenchOptions) {
  ...
  const primaryLabel = primaryStops
    ...
    : over?.running === true && steeringAvailable && !composerLocked && plainMessageDraft
      ? (over.busyEnter === 'steer' ? '插話發送' : '排隊發送')
      : '發送消息'
...
describe('image draft rail', () => {
  it('collects clipboard files while preserving text from a mixed paste', async () => {
    ...
    await vi.waitFor(() => { expect(shell.snapshot.draft).toBe('同時粘貼的文字') })
  })
  ...
  it('pre-checks projected limits at intake: whole-batch refusal with product copy, none added', () => {
    ...
    expect(overCount.view.getByRole('alert').textContent).toContain('一條消息最多添加 2 張圖片')
    ...
    expect(overFile.view.getByRole('alert').textContent).toContain('單張圖片不能超過 1MB')
    ...
    expect(overTotal.view.getByRole('alert').textContent).toContain('圖片總大小超過 2MB')
  ...
  it('announces the format problem before any limit when the batch holds a non-image', () => {
    const addFiles = vi.fn(() => '僅支持 PNG、JPG、WebP、GIF 格式的圖片')
    const result = bench({
      addFiles,
      imageLimits: {
        maxImageBytes: 8,
    ...
    expect(result.view.getByRole('alert').textContent).toContain('僅支持 PNG、JPG、WebP、GIF 格式的圖片')
  })
  ...
  it('announces server attachment rejections as product copy, other codes as developer text', () => {
    ...
    expect(model.view.getByRole('alert').textContent).toContain('當前模型不支持圖片，請切換支持圖片的模型')
    cleanup()
    const unknown = bench({ promptError: attachmentError('ATTACHMENT_NOT_REFERENCED') })
    expect(unknown.view.getByRole('alert').textContent).toContain('圖片發送失敗（ATTACHMENT_NOT_REFERENCED）')
    cleanup()
    ...
    expect(subagent.view.getByRole('alert').textContent).toContain('當前模型不支持圖片，請切換支持圖片的模型')
    cleanup()
    const other = bench({
  ...
  it('sends an image-only draft and exposes removal through the attachment slot', async () => {
    ...
    expect((view.getByRole('button', { name: '發送消息' }) as HTMLButtonElement).disabled).toBe(false)
  ...
  it('returns an image-only draft to the rail when its admission fails', async () => {
    ...
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(attachmentOwner(result.slotCalls).attachments).toEqual([])
    await act(async () => { fail({ kind: 'error', text: '圖片發送失敗' }) })
    await vi.waitFor(() => {
      expect(attachmentOwner(result.slotCalls).attachments).toEqual([attachments[0]])
    })
    expect(result.view.getByRole('alert').textContent).toContain('圖片發送失敗')
  })
  ...
  it('announces an image-intake rejection as a fading toast, repeatable for the same reason', () => {
    ...
    try {
      ...
      paste()
      expect(view.getByRole('alert').textContent).toContain('僅支持 PNG、JPG、WebP、GIF 格式的圖片')
  ...
  it('announces a rejected attachment-slot intake through the same toast', () => {
    const addFiles = vi.fn(() => '圖片讀取服務不可用')
    ...
    expect(result.view.getByRole('alert').textContent).toContain('圖片讀取服務不可用')
  })
})
...
describe('Enter semantics', () => {
  it('advertises the empty-draft whole-queue steering gesture when it is available', () => {
    ...
    expect(placeholder).toBe('Cmd/Ctrl+Enter 插話發送全部排隊消息')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      subagent: {
    ...
    }).placeholder).toBe('Cmd/Ctrl+Enter 插話發送全部排隊消息')
  })

  it('keeps the owning placeholder or ordinary guidance when whole-queue steering is unavailable', () => {
    expect(bench({ running: true }).placeholder).toBe('發消息或創建任務, / 調用指令, @ 文件或對話')
    expect(bench({ queue: [row('q-1')] }).placeholder).toBe('發消息或創建任務, / 調用指令, @ 文件或對話')
    expect(bench({ running: true, queue: [row('q-1')], draft: '消息' }).placeholder).toBe('發消息或創建任務, / 調用指令, @ 文件或對話')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      placeholder: '上層指定提示',
    }).placeholder).toBe('上層指定提示')
    ...
    }).placeholder).toBe('發消息或創建任務, / 調用指令, @ 文件或對話')
  ...
  it('draft content outranks the queue: accelerated Enter steers the draft only', () => {
    const steerQueue = vi.fn()
    const { textarea, sink } = bench({ running: true, queue: [row('q-1')], draft: '插話', steerQueue })
    fireEvent.keyDown(textarea, { key: 'Enter', ctrlKey: true })
    expect(sink).toHaveBeenCalledWith('插話', [], 'steer', expect.any(AbortSignal))
    expect(steerQueue).not.toHaveBeenCalled()
  })
...
describe('running and lock semantics', () => {
  it('dismisses the Stop tooltip when an empty composer becomes idle', () => {
    ...
    try {
      ...
      expect(button.disabled).toBe(true)
      expect(button.getAttribute('aria-label')).toBe('發送消息')
  ...
  it('running switches the primary between Stop and Queue Send with the draft', async () => {
    ...
    writeDraft(shell, '排隊消息')
    expect(button.getAttribute('aria-label')).toBe('排隊發送')
    writeDraft(shell, '   ')
    expect(button.getAttribute('aria-label')).toBe('停止生成')
    writeDraft(shell, '排隊消息2')
    expect(button.getAttribute('aria-label')).toBe('排隊發送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('排隊消息2', [], 'queue', expect.any(AbortSignal))
  ...
  it('running Send follows the busy-state Steer preference and labels the delivery', () => {
    const { button, sink } = bench({ running: true, busyEnter: 'steer', draft: '按鈕插話' })
    expect(button.getAttribute('aria-label')).toBe('插話發送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('按鈕插話', [], 'steer', expect.any(AbortSignal))
  })

  it('running Send relabels when the busy-state preference changes live', () => {
    const { button, busyEnter, sink } = bench({ running: true, draft: '跟隨設置' })
    expect(button.getAttribute('aria-label')).toBe('排隊發送')
    act(() => { busyEnter.set('steer') })
    expect(button.getAttribute('aria-label')).toBe('插話發送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('跟隨設置', [], 'steer', expect.any(AbortSignal))
  })
  ...
  it('running Send keeps the plain label for a slash line and a claimed command', () => {
    ...
    expect(slash.button.getAttribute('aria-label')).toBe('發送消息')
    ...
    const button = claimed.view.container.querySelector<HTMLButtonElement>('button[aria-label="發送消息"]')
    expect(button).not.toBeNull()
    expect(claimed.view.container.querySelector('button[aria-label="插話發送"]')).toBeNull()
  })
  ...
  it('running Send keeps the plain label while a file upload is still pending', () => {
    ...
    const pending = bench({
      running: true, busyEnter: 'steer', draft: '帶附件', attachments: [file],
      fileUploads: { [file.id]: { status: 'uploading', loaded: 0 } },
    })
    expect(pending.button.getAttribute('aria-label')).toBe('發送消息')
    ...
    expect(failed.button.getAttribute('aria-label')).toBe('發送消息')
    ...
    expect(ready.button.getAttribute('aria-label')).toBe('插話發送')
    expect(ready.button.disabled).toBe(false)
  })

  it('idle Send keeps the plain label regardless of the busy-state preference', () => {
    const { button, sink } = bench({ busyEnter: 'steer', draft: '空閑發送' })
    expect(button.getAttribute('aria-label')).toBe('發送消息')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('空閑發送', [], 'queue', expect.any(AbortSignal))
  })
  ...
  it('running blocked composer keeps Stop with a retained draft', () => {
    const { button, sink, stop, textarea } = bench({
      running: true,
      draft: '保留的草稿',
      blocked: { reason: '請選擇可用模型' },
      placeholder: '請選擇可用模型',
    })
    expect(textarea.getAttribute('aria-disabled')).toBe('true')
    expect(textarea.getAttribute('data-placeholder')).toBe('請選擇可用模型')
    expect(button.getAttribute('aria-label')).toBe('停止生成')
  ...
  it('running plain Enter follows the busy-state Steer preference', () => {
    const { textarea, sink } = bench({ running: true, busyEnter: 'steer', draft: '直接插話' })
    fireEvent.keyDown(textarea, { key: 'Enter' })
    expect(sink).toHaveBeenCalledWith('直接插話', [], 'steer', expect.any(AbortSignal))
  })

  it('running Cmd/Ctrl+Enter uses the opposite of the busy-state Enter preference', () => {
    const meta = bench({ running: true, busyEnter: 'steer', draft: '排到下一輪' })
    fireEvent.keyDown(meta.textarea, { key: 'Enter', metaKey: true })
    expect(meta.sink).toHaveBeenCalledWith('排到下一輪', [], 'queue', expect.any(AbortSignal))
  ...
  it('running continuable subagent keeps Send beside an independent Stop', () => {
    ...
    click.mockRestore()
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('后續消息', [], 'queue', expect.any(AbortSignal))
  ...
  it('running continuable subagent Send follows the Steer preference like an ordinary session', () => {
    ...
    const { button, sink } = bench({ running: true, busyEnter: 'steer', draft: '子代理插話', subagent })
    expect(button.getAttribute('aria-label')).toBe('插話發送')
    fireEvent.click(button)
    expect(sink).toHaveBeenCalledWith('子代理插話', [], 'steer', expect.any(AbortSignal))
    ...
    expect(empty.button.getAttribute('aria-label')).toBe('發送消息')
  ...
  it('parent-offline running continuable locks Send but keeps independent Stop usable', () => {
    ...
    expect(textarea.getAttribute('aria-disabled')).toBe('true')
    expect(placeholderOf(view.container)).toBe('父會話已離線，無法繼續發送；仍可停止當前運行')
    expect((view.getByLabelText('添加文件或調用指令') as HTMLButtonElement).disabled).toBe(true)
    expect(button.getAttribute('aria-label')).toBe('發送消息')
    expect(button.disabled).toBe(true)
  ...
  it('disabled (session removed) locks the textarea and chrome', () => {
    ...
    expect(textarea.getAttribute('aria-disabled')).toBe('true')
    expect(placeholderOf(view.container)).toBe('會話不可用')
    expect((view.getByLabelText('添加文件或調用指令') as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('unlock refocuses the surface; mousedown on the button keeps focus', () => {
    ...
    fireEvent.mouseDown(first.view.container.querySelector('button[aria-label="發送消息"]')!)
  ...
  it('disabled state shows the unavailable placeholder; custom placeholder wins', () => {
    expect(bench({ disabled: true }).placeholder).toBe('會話不可用')
    const live = bench()
    expect(live.placeholder).toBe('發消息或創建任務, / 調用指令, @ 文件或對話')
...
describe('machine pending lock', () => {
  it('submitting renders read-only textarea, pending dot, and a disabled primary', () => {
    ...
    const textarea = view.container.querySelector<HTMLDivElement>('[data-composer-input]')!
    expect(editableOf(textarea)).toBe(false)
    expect(view.container.querySelector<HTMLButtonElement>('button[aria-label="發送消息"]')!.disabled).toBe(true)
  })
})
...
describe('decorations', () => {
  ...
  it('claimed token styles the leading leaf and sets the blank-args hint variable', () => {
    ...
    act(() => {
      shell.setDraft('/goal ')
      shell.beginCommand(
        { name: 'goal', token: '/goal ', hint: '目標內容', submit: () => Promise.resolve({ kind: 'success' as const }) },
        { start: 0, end: 6, draftRev: shell.snapshot.draftRev },
      )
    ...
    expect(textarea.style.getPropertyValue('--dsh-composer-hint')).toBe(JSON.stringify('目標內容'))
    // Args typed: the hint disappears, the token style stays.
    act(() => { shell.setDraft('/goal 發布') })
  ...
  it('a locale entry for the claimed command overrides the raw claim hint (trailing-space token)', () => {
    ...
    expect(textarea.style.getPropertyValue('--dsh-composer-hint')).toBe(JSON.stringify('輸入目標，智能體將持續執行'))
  })
  ...
  it('the hint lookup keys on the claim name, so a localized claim token keeps the locale entry', () => {
    ...
    act(() => {
      shell.setDraft('/目標 ')
      shell.beginCommand(
        { name: 'goal', token: '/目標 ', hint: '[<objective>|clear|edit <objective>|pause|resume]', submit: () => Promise.resolve({ kind: 'success' as const }) },
  ...
  it('suppresses placeholders throughout native composition, including a temporarily empty draft', async () => {
    ...
    act(() => {
      shell.setDraft('/目標 ')
      shell.beginCommand(
        { name: 'goal', token: '/目標 ', hint: '目標內容', submit: () => Promise.resolve({ kind: 'success' as const }) },
        { start: 0, end: 4, draftRev: shell.snapshot.draftRev },
      )
    })
    fireEvent.compositionStart(textarea)
    act(() => { shell.setDraft('/目標 z') })
    act(() => { shell.setDraft('/目標 ') })
    expect(textarea.hasAttribute('data-composer-composing')).toBe(true)
    fireEvent.compositionEnd(textarea, { data: '這' })
    act(() => { shell.setDraft('/目標 這') })
    await act(async () => {})
    expect(textarea.hasAttribute('data-composer-composing')).toBe(false)
    expect(textarea.style.getPropertyValue('--dsh-composer-hint')).toBe('')
  })
  ...
  it('an inserted reference renders a real chip capsule with its icon and label', () => {
    const { view, shell } = bench()
    const reference = {
      source: 'reference', ref: 'w1', label: '會話一', appearance: 'session' as const, clipboardText: '@w1',
    }
    act(() => {
      shell.setDraft('參考 @w1 內容')
  ...
  it('keeps the chip decorator mounted when the session becomes disabled', () => {
    ...
    act(() => {
      shell.setDraft('@w1')
      shell.insertReference({
        source: 'reference', ref: 'w1', label: '會話一', appearance: 'session', clipboardText: '@w1',
...
describe('strips and variants', () => {
  ...
  it('announces an error notice from the machine store as a fading toast', () => {
    ...
    try {
      const { view, shell } = bench()
      act(() => { shell.notify('error', '命令失敗了') })
      expect(view.getByRole('alert').textContent).toContain('命令失敗了')
...
describe('command launcher chrome and control seats', () => {
  it('renders the command launcher; the Access chip is absent without the permissions projection; the control seats render EMPTY without entries', () => {
    ...
    expect(view.getByLabelText('添加文件或調用指令')).toBeTruthy()
    // Capability absent (no projection value): the chip renders nothing.
    expect(view.queryByLabelText(/^訪問模式/)).toBeNull()
  ...
  it('passes the textarea selection to the command menu launcher and reflects its expanded state', () => {
    ...
    const launcher = view.getByLabelText('添加文件或調用指令')
  ...
  it('requires explicit risk acknowledgement before submitting full access', async () => {
    ...
    const { view } = bench({ permissions, command })
    fireEvent.click(view.getByLabelText(/^訪問模式/))
    fireEvent.click(view.getByRole('menuitem', { name: '完全權限' }))

    expect(command).not.toHaveBeenCalled()
    expect(view.getByRole('dialog', { name: '確認啟用完全權限？' })).toBeTruthy()
    const enable = view.getByRole('button', { name: '啟用完全權限' }) as HTMLButtonElement
    expect(enable.disabled).toBe(true)

    fireEvent.click(view.getByRole('checkbox', { name: '我已了解風險，并愿意繼續' }))
    ...
    expect((view.getByLabelText(/^訪問模式/) as HTMLButtonElement).textContent).toBe('完全權限')
    await act(async () => {})
  })
  ...
  it('cancels a full access selection without changing permission and resets acknowledgement', () => {
    ...
    expect(command).not.toHaveBeenCalled()
    expect((view.getByLabelText(/^訪問模式/) as HTMLButtonElement).textContent).toBe('工作區內修改')

    openConfirmation()
    expect((view.getByRole('checkbox') as HTMLInputElement).checked).toBe(false)
    expect((view.getByRole('button', { name: '啟用完全權限' }) as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('disabled locks the Access chip and command launcher (running does not)', () => {
    ...
    expect((view.getByLabelText('添加文件或調用指令') as HTMLButtonElement).disabled).toBe(true)
    expect((view.getByLabelText(/^訪問模式/) as HTMLButtonElement).disabled).toBe(true)
    cleanup()
    const live = bench({ running: true, permissions })
    expect((live.view.getByLabelText(/^訪問模式/) as HTMLButtonElement).disabled).toBe(false)
  })
})
