...
const ARGS = '{"name":"clock","purpose":"頂欄時鐘","code":{"client":"return {}","host":"harness.handle(\'now\', () => Date.now())"}}'
...
describe('cordisDefineCard', () => {
  it('reads name, purpose and both code halves off the call arguments', () => {
    ...
    expect(card).toMatchObject({
      name: 'clock', purpose: '頂欄時鐘', clientCode: 'return {}', state: 'running', output: null,
    })
  ...
  it('keeps the raw first line as the name when the arguments carry none', () => {
    expect(cordisDefineCard(running({ argsRaw: '{"purpose":"頂欄時鐘"}' })).name).toBe('{"purpose":"頂欄時鐘"}')
    expect(cordisDefineCard(running({ argsRaw: '{"name":"","purpose":"頂欄時鐘"}' })).name).toBe('{"name":"","purpose":"頂欄時鐘"}')
  })
