...
describe('reading display metadata', () => {
  it('reads a name and a description', async () => {
    const dir = await presetDir('name: 標準模式\ndescription: 完整的編碼 agent。\n')

    expect(await readPresetMetadata(dir)).toEqual({ name: '標準模式', description: '完整的編碼 agent。' })
  })
  ...
  it('trims surrounding whitespace', async () => {
    const dir = await presetDir('name: "  極簡模式  "\n')

    expect(await readPresetMetadata(dir)).toEqual({ name: '極簡模式' })
  })

  it('reads a declared order', async () => {
    const dir = await presetDir('name: 標準模式\norder: 1\n')

    expect(await readPresetMetadata(dir)).toEqual({ name: '標準模式', order: 1 })
  })
...
describe('rendering display metadata', () => {
  it('round-trips through a read', async () => {
    const rendered = renderPresetMetadata({ name: '創造模式', description: '可以改自己的組裝。' })
    const dir = await presetDir(rendered)

    expect(await readPresetMetadata(dir)).toEqual({ name: '創造模式', description: '可以改自己的組裝。' })
  })

  it('stores a declared order', () => {
    expect(renderPresetMetadata({ name: '標準模式', order: 1 })).toBe('name: 標準模式\norder: 1\n')
  })

  it('omits an absent field rather than writing it blank', () => {
    expect(renderPresetMetadata({ name: '極簡模式' })).toBe('name: 極簡模式\n')
    // Description without a name is legal too: the picker falls back to the id.
    expect(renderPresetMetadata({ description: '只做檢索。' })).toBe('description: 只做檢索。\n')
  })
